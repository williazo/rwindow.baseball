...
#'
#'Scrape data from \url{https://www.thebaseball.cube} to look at the top 100 prospects list for each team or the top prospects by team.
...
prospect_cube <- function(team, year, src = "BA", start_year = NULL, end_year = NULL){
  ...
  if(missing(year)==FALSE & missing(team)==FALSE){
  ...
  url <- paste0(base_url, year, "&Src=", src)
